...
def removeInsideBrackets(st):
  res = ""
  bit = False
  for i in range(len(st)):
    if(i+1<len(st) and st[i:(i+2)] == ' ('):
      bit = True
    if not bit:
      res += st[i]
    if(i-1>=0 and st[(i-1):(i+1)] == ') '):
      bit = False
  return res
...
def parse_time(timeStr):
  ...
  for part in timeStr.split(" ,"):
    part = removeInsideBrackets(part);
...
csv_file = open("all_courses_even.csv")
...
with open("course_list_even.json", 'w') as write_json_file:
